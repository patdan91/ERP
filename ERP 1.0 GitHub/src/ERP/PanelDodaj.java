...
public class PanelDodaj extends JPanel{
	...
	private JLabel lNumer = new JLabel("Podaj numer transporu");
	private JTextField tNumer = new JTextField("");
	private Date data = new Date();
	private SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd"); 
	private JLabel lData = new JLabel("Data transportu:");
	...
	//Tworzenie okna z mo¿liwoœci¹ dodawania dostaw drewna
	public PanelDodaj() {
		...
		sPoleTekstowe.setAlignmentX(Component.CENTER_ALIGNMENT);
	...
	//sprawdzenie prowid³owej iloœci wprowadzonego drewna
	...
	class DodajListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			//sprawdzenie prowid³owej iloœci wprowadzonego drewna oraz czy wprowadzono numer dostawy
			//sprawdzenie czy dostawa o podany numerze jest ju¿ w tabeli "dostawa". Jeœli nie to dodajemy rekord. Nastêpnie sprawdzana jest tabela z konkretnym przekrojem drewna.
			// Jeœli w tabeli znajduje siê ju¿ rekord z kluczem obcym o numerze dostawy to rekord jest aktualizowany, jeœli nie to rekord jest dodawany.
			
			if(isInteger(tIlosc.getText())&& !tNumer.getText().isEmpty()) {
			String zapis;
			if(s.sprawdzDostawe2(grubosc[cGrubosc.getSelectedIndex()], szerokosc[cSzerokosc.getSelectedIndex()], tNumer.getText())==1) {
				zapis= s.aktualizujDrewno(grubosc[cGrubosc.getSelectedIndex()], szerokosc[cSzerokosc.getSelectedIndex()], tNumer.getText(), dlugosc[cDlugosc.getSelectedIndex()], tIlosc.getText(), tData.getText());
				poleTekstowe.append(zapis);
				historia.zapiszHistorie(zapis);
			}
			else {
				if(s.sprawdzDostawe1(tNumer.getText())==1) {
					zapis= s.dodajDrewno(grubosc[cGrubosc.getSelectedIndex()], szerokosc[cSzerokosc.getSelectedIndex()], tNumer.getText(), dlugosc[cDlugosc.getSelectedIndex()], tIlosc.getText(), tData.getText());
					poleTekstowe.append(zapis);
					historia.zapiszHistorie(zapis);
				}
				else {
					s.dodajDostawe(tNumer.getText(), tData.getText());
